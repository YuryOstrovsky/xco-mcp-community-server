    ...
    **kwargs,
    ...
    if not in_window and status == "failed" and fallback_detect_non_success:
        ...
        if int(tier1_all.get("status") or 500) == 200:
            all_items_raw = [x for x in _as_list(tier1_all.get("payload")) if isinstance(x, dict)]
            all_items = _filter_window_and_query(all_items_raw)
            all_items = [ex for ex in all_items if _looks_failed(ex.get("status"))]

            in_window = all_items
            used_fallback = True
            effective_items = all_items_raw
    ...
    # ✅ Optional polish: explain why fallback happened when primary returned nothing but we still see executions.
    if used_fallback and len(primary_items) == 0 and len(effective_items) > 0:
        warnings.append(
            "Backend returned no executions for status=failed filter; used status=all + local failure detection."
        )
